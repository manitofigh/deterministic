...
int main(int argc, char **argv)
{
    ...
    parse_command_line(argc, argv, &num_runs, &initial, &machine, &spec2k6, &num_benchmarks);
    ...
    for (j = 0; j < num_benchmarks; j++) {
        /* 100 */
        result =
            parse_file(argv[(j * 2) + BENCH_START], argv[(j * 2) + BENCH_START + 1], "100", run,
                       machines[machine].name, &stats_100[j].instrs[machine][run],
                       &stats_100[j].cycles[machine][run], &stats_100[j].time_ns[machine][run]);

        if (result < 0)
            stats_100[j].valid[machine][run] = 0;
        else
            stats_100[j].valid[machine][run] = 1;

        /* 250 */
        result =
            parse_file(argv[(j * 2) + BENCH_START], argv[(j * 2) + BENCH_START + 1], "250", run,
                       machines[machine].name, &stats_250[j].instrs[machine][run],
                       &stats_250[j].cycles[machine][run], &stats_250[j].time_ns[machine][run]);

        if (result < 0)
            stats_250[j].valid[machine][run] = 0;
        else
            stats_250[j].valid[machine][run] = 1;

        /* 1000 */
        result =
            parse_file(argv[(j * 2) + BENCH_START], argv[(j * 2) + BENCH_START + 1], "1000", run,
                       machines[machine].name, &stats_1000[j].instrs[machine][run],
                       &stats_1000[j].cycles[machine][run], &stats_1000[j].time_ns[machine][run]);

        if (result < 0)
            stats_1000[j].valid[machine][run] = 0;
        else
            stats_1000[j].valid[machine][run] = 1;
    }
  ...
  */
    ...
    printf("\n(* Title *)\n");
    printf(
        "title fontsize 14 font Helvetica : Extra Instruction Counts on %s, divided by Runtime (%s)\n",
    ...
    for (i = 0; i < num_benchmarks; i++) {
        ...
        if (value > 0.0)
            printf("%d %lf (* %lf %s.%s *)\n", i, log10(value), value, argv[(i * 2) + BENCH_START],
    ...
    for (i = 0; i < num_benchmarks; i++) {
        ...
        if (value > 0.0)
            printf("%d %lf (* %lf %s.%s *)\n", i + num_benchmarks, log10(value), value,
                   argv[(i * 2) + BENCH_START], argv[(i * 2) + BENCH_START + 1]);
    }
    ...
    for (i = 0; i < num_benchmarks; i++) {
        ...
        if (value > 0.0)
            printf("%d %lf (* %lf %s.%s *)\n", i + (num_benchmarks * 2), log10(value), value,
                   argv[(i * 2) + BENCH_START], argv[(i * 2) + BENCH_START + 1]);
    }
        ...
        if (seconds > 0.0) {
            ...
            if ((value > 0.0) && (fabs((predicted - value) / predicted) > error)) {
                printf("newstring vjc hjl x %d y %f fontsize 10 font Helvetica : %s.%s\n", i + 1,
                       log10(value), argv[(i * 2) + BENCH_START], argv[(i * 2) + BENCH_START + 1]);
            }
        }
    }
    ...
    for (i = 0; i < num_benchmarks; i++) {
        ...
        if (seconds > 0.0) {
            ...
            if ((value > 0.0) && (fabs((predicted - value) / predicted) > error)) {
                printf("newstring vjc hjl x %d y %f fontsize 10 font Helvetica : %s.%s\n",
                       num_benchmarks + i + 1, log10(value), argv[(i * 2) + BENCH_START],
                       argv[(i * 2) + BENCH_START + 1]);
            }
        }
    }
    ...
    for (i = 0; i < num_benchmarks; i++) {
        ...
        if (seconds > 0.0) {
            ...
            if ((value > 0.0) && (fabs((predicted - value) / predicted) > error)) {
                printf("newstring vjc hjl x %d y %f fontsize 10 font Helvetica : %s.%s\n",
                       (num_benchmarks * 2) + i + 1, log10(value), argv[(i * 2) + BENCH_START],
                       argv[(i * 2) + BENCH_START + 1]);
            }
        }
    }
