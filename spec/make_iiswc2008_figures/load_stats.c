...
static long long find_middle(long long *array, int num)
{
    ...
    qsort(blah, num, sizeof(long long), long_long_comp);
...
int parse_file(char *bench, char *input, char *tree, int which, char *machine, long long *instrs,
               long long *cycles, long long *ns)
{
    FILE *fff;
   ...
   if ((!strcmp(bench,"179.art"))) {
    ...
    sprintf(filename,
            ...
            DATA_SOURCE, tree, which, machine, bench, input, compiler);
...
static int parse_valgrind(char *bench, char *input, char *tree, int which, long long *instrs,
...
static int parse_qemu(char *bench, char *input, char *tree, int which, long long *instrs, int final)
{
    FILE *fff;
    char string[BUFSIZ], filename[BUFSIZ];
    char *result;
...
static int parse_pin(char *bench, char *input, char *tree, int which, long long *instrs, int final)
{
    FILE *fff;
    char string[BUFSIZ], filename[BUFSIZ];
    char *result;
...
void load_stats(int num_runs, int spec2k6, struct stats *our_stats, char *bench_name, char *input,
                char *tree, int benchmark, int fixup)
{
    ...
    for (run = 0; run < num_runs; run++) {
        for (i = 0; i < REAL_MACHINES; i++) {
            if (machine_valid(i, spec2k6)) {
                result = parse_file(bench_name, input, tree, run, machines[i].name,
                                    &our_stats[benchmark].instrs[i][run],
                                    &our_stats[benchmark].cycles[i][run],
                                    &our_stats[benchmark].time_ns[i][run]);

                if (result < 0)
                    our_stats[benchmark].valid[i][run] = 0;
                else
                    our_stats[benchmark].valid[i][run] = 1;
            }
        }
        ...
        if (result < 0)
            our_stats[benchmark].valid[VALGRIND][run] = 0;
        else
            our_stats[benchmark].valid[VALGRIND][run] = 1;

        /* Load pin results */
        result =
            parse_pin(bench_name, input, tree, run, &our_stats[benchmark].instrs[PIN][run], final);

        if (result < 0)
            our_stats[benchmark].valid[PIN][run] = 0;
        else
            our_stats[benchmark].valid[PIN][run] = 1;

        /* Load qemu results */
        result = parse_qemu(bench_name, input, tree, run, &our_stats[benchmark].instrs[QEMU][run],
                            final);

        if (result < 0)
            our_stats[benchmark].valid[QEMU][run] = 0;
        else
            our_stats[benchmark].valid[QEMU][run] = 1;
    }
    ...
    for (i = 0; i < MAX_MACHINES; i++) {
        ...
        for (j = 0; j < num_runs; j++) {
            if (our_stats[benchmark].valid[i][j]) {
                stdev += (double)(our_stats[benchmark].instrs[i][j] -
                                  our_stats[benchmark].machine_mean[i]) *
                         (double)(our_stats[benchmark].instrs[i][j] -
                                  our_stats[benchmark].machine_mean[i]);
            }
        }
    ...
    for (i = 0; i < MAX_MACHINES; i++) {
        our_stats[benchmark].instrs_mid[i] = find_middle(our_stats[benchmark].instrs[i], num_runs);
    }

    for (i = 0; i < MAX_MACHINES; i++) {
        our_stats[benchmark].diffs[i] =
            our_stats[benchmark].instrs_mid[i] - our_stats[benchmark].instrs_mid[PIN];
    }
}
