    ...
    for (i = 0; i < num_machines; i++) {
        ...
        for (which = 0; which < num_runs; which++) {
            for (j = 0; j < NUM_BENCHMARKS; j++) {
                parse_file(benchmarks[j].name, benchmarks[j].input, argv[TREE], which,
                           machines[i].name, &instrs, &cycles, &nanosecs);
        ...
        printf("%s & %.0f & %.0f & %.0f & %.1f\\%%\\\\\n", machines[i].processor2,
               ...
               (machines[i].real_MHz - mean_MHz) / machines[i].real_MHz * 100.0);
    }
