...
int main(int argc, char **argv)
{
    ...
    for (j = 0; j < MAX_MACHINES; j++) {
        if (!(!machines[j].is_spec2k6 && spec2k6)) {
            ...
            for (k = 0; k < num_runs; k++) {
                result = (double)(init_stats[0].instrs[j][k] - init_stats[0].machine_mean[j]);
    ...
    for (j = 0; j < MAX_MACHINES; j++) {
        if (!(!machines[j].is_spec2k6 && spec2k6)) {
            ...
            for (k = 0; k < num_runs; k++) {
                result = (double)(final_stats[0].instrs[j][k] - final_stats[0].machine_mean[j]);
