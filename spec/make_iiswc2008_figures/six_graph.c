...
int main(int argc, char **argv)
{
    ...
    parse_command_line(argc, argv, &num_runs, &initial, &legend, &spec2k6, &num_benchmarks);
...
#define LEGEND_YSCALE 1.4
    ...
    for (i = 0; i < num_benchmarks; i++) {
        ...
        printf("hash_label at %f : %s.%s\n",
               // (i*WIDTH)+half,
               (i * width) + half - adjust_2k6_half, argv[(i * 2) + BENCH_START],
    ...
    if (!initial) {
        printf(
            ...
            (double)(num_benchmarks * width) / BIGMARK_SCALE, 1.0, 14.0);
        ...
        for (i = 0; i < num_benchmarks; i++) {
            if (init_stats[i].bench_stdev != 0.0) {
                printf("%f %.9f\n", (i * width) + half - adjust_2k6_half,
                       log10(init_stats[i].bench_stdev));
            }
        }
    ...
    printf("\nnewcurve color 0.85 0.85 0.85 marktype xbar marksize %f "
           ...
           (double)(num_benchmarks * width) / SMALLMARK_SCALE, 1.0, 12.0);
    ...
    for (i = 0; i < num_benchmarks; i++) {
        if (initial) {
            ...
            printf("%f %.9f\n", (i * width) + half - adjust_2k6_half,
                   log10(final_stats[i].bench_stdev));
        }
    }
    ...
    for (j = adjust_2k6; j < MAX_MACHINES; j++) {
        if (!(!machines[j].is_spec2k6 && spec2k6)) {
            ...
            printf("label hjl vjc fontsize 12 font Helvetica "
                   ...
                   (((double)((3 - (j % 3)))) * LEGEND_YSCALE) + 10.5, machines[j].processor2);
