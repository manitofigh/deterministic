...
#define LEGEND_YSCALE 1.0
    ...
    for (i = 0; i < num_benchmarks; i++) {
        ...
        result = 100.0 * (init_stats[i].bench_stdev) / (double)init_stats[i].bench_mean;
    ...
    for (i = 0; i < num_benchmarks; i++) {
        ...
        if ((valid) && (result != 0.0)) {
            ...
            printf("(* %s.%s %lld %lf *)\n", argv[(i * 2) + BENCH_START],
                   argv[(i * 2) + BENCH_START + 1], init_stats[i].bench_mean,
                   init_stats[i].bench_stdev);
        }
    }
    ...
    for (i = 0; i < num_benchmarks; i++) {
        ...
        result = (final_stats[i].bench_stdev) / (double)final_stats[i].bench_mean;
