...
#define BENCHMARK_SLOTS 12.0
    ...
    printf("label font Helvetica fontsize 14 x -%lf : %s\n",
           ((double)num_benchmarks) * BENCHMARK_SLOTS / 40.0, yaxis);
   ...
   printf("hash_at -12 hash_label at -12 : -1T\n");
    ...
    for (i = 0; i < MAX_MACHINES; i++) {
        printf("hash_label at %i : %s\n",
               ...
               (i * WIDTH) + HALF, machines[i].processor2);
    }
                    ...
                    if (valid) {
                        ...
                        if (result < 0.0) {
                            printf("%lf %.9f (* %s.%s *)\n", (j * WIDTH) + (i * scale) + OFFSET,
                                   -log10(fabs(result)), argv[BENCH_START + (i * 2)],
                                   argv[BENCH_START + (i * 2) + 1]);
                        } else if (result > 0.0) {
                            printf("%lf %.9f (* %s.%s *)\n", (j * WIDTH) + (i * scale) + OFFSET,
                                   log10(result), argv[BENCH_START + (i * 2)],
                                   argv[BENCH_START + (i * 2) + 1]);
                        } else {
                            printf("%lf 0.0 (* %s.%s *)\n", (j * WIDTH) + (i * scale) + OFFSET,
                                   argv[BENCH_START + (i * 2)], argv[BENCH_START + (i * 2) + 1]
