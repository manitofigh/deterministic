...
HARDWARE_EVENTS = {
    'cycles': 0, 'cpu-cycles': 0, 'instructions': 1,
    'cache-references': 2, 'cache-misses': 3, 'branches': 4,
    'branch-instructions': 4, 'branch-misses': 5, 'bus-cycles': 6,
    'stalled-cycles-frontend': 7, 'stalled-cycles-backend': 8, 'ref-cycles': 9,
}
CACHE_EVENTS = {
    'L1-dcache': 0, 'L1-icache': 1, 'LLC': 2, 'dTLB': 3,
    'iTLB': 4, 'branch': 5, 'node': 6,
}
...
def encode_event(event, records):
    ...
    return (int((device / 'type').read_text()),
            config['config'], config['config1'], config['config2'])
...
def prepare(events, output):
    ...
    command = [compiler, '-O2', '-std=gnu11', '-Wall', '-Wextra', '-Werror',
    ...
    result = subprocess.run(['perf', 'list', '--json'], capture_output=True, text=True,
                            env={**os.environ, 'LC_ALL': 'C', 'PERF_PAGER': 'cat'}, timeout=60)
...
def trial(helper, encoding, period, benchmark, folder, log, timeout):
    raw_path = folder / 'perf.tmp'
    command = [str(helper), *map(str, encoding), str(period), str(raw_path), str(benchmark)]
...
def test_event(index, event, cpu, folder, benchmark, rounds, timeout,
               counts_file, helper, config, requested_period):
    ...
    with log_path.open('w', buffering=1) as log:
        ...
        if not error and period is None:
            log.write('\n--- preliminary count ---\n')
        ...
        if error:
            ...
            for number in range(1, rounds + 1):
                log.write(f'\n--- round {number} ---\n')
                result = trial(helper, config['encoding'], period, benchmark, folder, log, timeout)
                result['round'] = number
                ...
                if result.get('error'):
                    ...
                    log.write(f'count at stop: {result["count"]}\n'
                              f'skid: {result["skid"]}\n'
                              f'instruction pointer: {result["ip"]}\n')
                counts_file.write(f'{event}:u\t{number}\t{period}\t'
...
def run_worker(cpu, assigned, folder, benchmark, rounds, timeout, helper, configs, period):
    signal.signal(signal.SIGINT, signal.SIG_IGN)
    signal.signal(signal.SIGTERM, interrupt)
    try:
        os.sched_setaffinity(0, {cpu})
        with (folder / 'counts.tsv').open('w', buffering=1) as counts:
            counts.write('event\tround\tperiod\tcount\tskid\tip\terror\n')
            with (folder / 'results.jsonl').open('w', buffering=1) as results:
                for index, event in assigned:
                    result = test_event(index, event, cpu, folder, benchmark, rounds,
                                        timeout, counts, helper, configs[event], period)
...
def report_rows(rows):
    ...
    text = ['Skid is extra event occurrences before the Linux process stop.', '',
    ...
    if errors:
        ...
        for row in errors:
            text.append(f'| [`{row["event"]}`]({row["log"]}) | '
                        f'{len(row["skids"])} | {row["errors"]} |')
    text.append('')
    return text
