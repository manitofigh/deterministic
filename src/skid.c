...
static const char *measure(pid_t child, int fd, int signo, uint64_t overflow_threshold,
                          struct counter_value *value, uint64_t *ip)
{
    ...
    if ((status >> 16) == PTRACE_EVENT_EXIT) {
        ...
        if (!overflow_threshold || WSTOPSIG(status) != signo ||
...
int main(int argc, char **argv)
{
    ...
    if (argc != 8) {
        fprintf(
            stderr,
            "usage: skid-helper type config config1 config2 overflow_threshold output benchmark\n");
    ...
    if (numbers[0] > UINT32_MAX || numbers[4] == 1 || numbers[4] > INT32_MAX) {
        fprintf(stderr, "invalid counter type or overflow threshold (use 2..2147483647)\n");
        return 2;
    }
    ...
    if (!error && value.count < attr.sample_period)
        error = "counter stopped below the requested overflow threshold";
...
out:
    ...
    if (error) {
        ...
        fprintf(output,
                "{\"count\":%" PRIu64 ",\"skid\":%" PRIu64 ",\"ip\":\"0x%" PRIx64
                "\",\"enabled\":%" PRIu64 ",\"running\":%" PRIu64 "}\n",
                value.count, (uint64_t)(attr.sample_period ? value.count - attr.sample_period : 0),
