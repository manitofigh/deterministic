...
def classify(event):
    ...
    unit = event.get('Unit', '')
    description = ' '.join(
        event.get(key, '') for key in ('BriefDescription', 'PublicDescription')
    ).lower()
...
def main():
    ...
    parser.add_argument(
        '--output', type=Path, help='event file (default: events/intel/<microarch>/events.txt)'
    )
    ...
    with user_owned_outputs(outputs):
        ...
        if args.input:
            ...
            if result.returncode or result.stderr.strip():
                raise ValueError(
                    f'perf list failed or reported a problem; see '
                    f'{stderr_path}; '
