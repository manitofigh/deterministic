...
char machine_names[MAX_NAME][BUFSIZ] = {
    "UNKNOWN",  "Pentium D", "Pentium 4",  "Phenom",      "Istanbul",
    "Core2",    "Nehalem",   "Nehalem-EX", "SandyBridge", "AMD Fam14h",
    "Westmere", "IvyBridge", "Atom",       "Haswell",     "AMD Fam15h",
};
...
void adjust_for_hw_interrupts(int j)
{
    ...
    for (i = 0; i < 10; i++) {
        ...
        if (hw_interrupts[i] < 1) {
            hwint_from_proc = 1;
            hw_interrupts[i] =
                stats[j].after_interrupts[i].total[0] - stats[j].before_interrupts[i].total[0];
        }
    }
...
static void read_interrupts(FILE *fff, int *cpus, struct interrupts *intr, int run)
{
    int i;
    ...
    *cpus = 0;
    ...
    while (1) {
        ...
        for (i = 0; i < *cpus; i++) {
            ...
            if (ptr != NULL)
                temp_interrupts = strtol(ptr, &endptr, 10);
...
static int read_stats(char *machine_type, int which, char *filename, char *benchmark_type,
                      int which_stat)
{
    char path[BUFSIZ];
    ...
    sprintf(path, "./%s/%d/%s.%s", machine_type, which, filename, benchmark_type);
    fff = fopen(path, "r");
    ...
    if (first_header) {
        ...
        if (interface == INTERFACE_PERF_EVENT)
            printf("perf_event\n");
        else if (interface == INTERFACE_PERFMON)
            printf("perfmon2\n");
        else
            printf("Unknown\n");
    ...
    for (i = 0; i < runs; i++) {
        while (1) {
            if (!strncmp(string, "### Interrupts", 14))
                break;
        ...
        read_interrupts(fff, &cpuinfo.num_cpus, stats[which_stat].before_interrupts, i);
        ...
        read_interrupts(fff, &cpuinfo.num_cpus, stats[which_stat].after_interrupts, i);
...
int main(int argc, char **argv)
{
    ...
    if (!strncmp(bench_name, "fp", 2))
        bench_type = BENCH_FP;
    else if (!strncmp(bench_name, "int", 3))
        bench_type = BENCH_INT;
    else if (!strncmp(bench_name, "sse", 3))
        bench_type = BENCH_SSE;
    else if (!strncmp(bench_name, "all", 3))
    ...
    /* detect machine */
    if (!strncmp(argv[1], "pentiumd", 8))
        machine_type = PENTIUMD;
    else if (!strncmp(argv[1], "pentium4", 8))
        machine_type = PENTIUM4;
    else if (!strncmp(argv[1], "phenom", 6))
        machine_type = PHENOM;
    else if (!strncmp(argv[1], "istanbul", 8))
        machine_type = ISTANBUL;
    else if (!strncmp(argv[1], "fam10h", 6))
        machine_type = ISTANBUL;
    else if (!strncmp(argv[1], "core2", 5))
        machine_type = CORE2;
    else if (!strncmp(argv[1], "nehalem-ex", 10))
        machine_type = NEHALEMEX;
    else if (!strncmp(argv[1], "nehalem", 7))
        machine_type = NEHALEM;
    else if (!strncmp(argv[1], "atom", 4))
        machine_type = ATOM;
    else if (!strncmp(argv[1], "sandybridge", 11))
        machine_type = SANDYBRIDGE;
    else if (!strncmp(argv[1], "fam14h", 6))
        machine_type = FAM14H;
    else if (!strncmp(argv[1], "westmere", 7))
        machine_type = WESTMERE;
    else if (!strncmp(argv[1], "ivybridge", 9))
        machine_type = IVYBRIDGE;
    else if (!strncmp(argv[1], "haswell", 7))
        machine_type = HASWELL;
    else if (!strncmp(argv[1], "fam15h", 6))
        machine_type = FAM15H;
    else
    ...
    re_loop:
        ...
        for (i = 0; i < RUNS; i++) {
            ...
            if (stats[j].value1[i] > max)
                max = stats[j].value1[i];

            if (stats[j].value1[i] < min)
                min = stats[j].value1[i];
        }
        ...
        if ((stats[j].raw_average - stats[j].expected[bench_type] == 0) && range == 0) {
            printf("\n\t**** DETERMINISTIC ****\n");
            deterministic = 1;
        } else {
            hwint_adjust = 0;
